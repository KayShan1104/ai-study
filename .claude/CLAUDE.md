...
- 不要先回答"等一下再记"——回答的同时就写
- 如果不确定是否该记，**直接问用户"这条要不要记到笔记？"**，不要自行判断跳过
- **追问和回答也是学习内容**：不要区分"主动讲解"和"回答追问"——用户追问、我回答，这轮对话中产生的新知识、新理解、新概念澄清，当场就记，不累积、不等待用户提醒
- **生成代码 + 讲解知识点 = 必须记录**：每个 step 的代码生成后，紧接着讲解涉及的新概念、新机制、实验观察，这些知识点必须在同一次回复中追加到笔记——不要等用户提问了才补记
- **失败模式**：不要在讲解知识点时说"我讲给你听"，然后忘了记笔记。讲解的同时就记，把讲解内容提炼后写入笔记
- **代码运行后必须记录**：写完 step 代码 → 运行 → 运行结果出来后 → 提炼涉及的知识点 → 记笔记 → 再开始跟用户讨论。不要把"运行结果分析"和"讨论"混在一起，导致忘了记录
- **追问补充已有笔记**：用户就之前记录的知识点追问、产生新理解时，不要只回答不更新——要把新的理解/修正/补充直接更新到笔记中对应的位置，保持笔记完整
...
- 只 commit，不 push
- **触发时机 1**：用户说"开始 step X"或"做 step X"时，第一件事是检查是否有未提交的变更，如有则先 commit
- **触发时机 2**：上一步完成、主动推进到下一步时（如"推进到 step X"、"继续到 step X"、"写 step X 代码"），第一件事是检查当前是否有未提交的变更（上一步的代码+笔记+规则修改），如有则先 commit
