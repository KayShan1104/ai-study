...
**Milestone 0**：
- [ ] 能列出 5 个 Agent 测试与传统测试的关键差异
- [ ] 能说出 3 个可以迁移的测试经验和 3 个必须抛弃的思维定式
- [ ] 写一段 200 字左右的总结：Agent 测试的核心挑战是什么
...
**Milestone 1**：
- [ ] 为一个具体的 Agent 场景设计 5+ 个评估指标
- [ ] 每个指标都有明确的定义、计算方式、评分标准
- [ ] 用 LLM-as-judge 跑通一次评估，输出评分结果
- [ ] 能解释 LLM-as-judge 的 3 个主要局限
...
**Milestone 2**：
- [ ] 100+ 个测试用例，覆盖 4+ 种类型
- [ ] 每个用例都有完整的标注（预期行为、输出特征、难度）
- [ ] 数据集用版本控制管理
- [ ] 有覆盖度分析（哪些场景覆盖充分、哪些不足）
