...
**Milestone 4**：
- [x] 端到端跑通：提问 → 检索 → 回答，回答基于检索内容
- [x] 问一个知识库里没有的问题，Agent 应该承认不知道而不是瞎编
- [x] 能打印出检索到了哪些 chunk（让用户看到"我参考了这些信息"）
...
- [x] 有 15+ 个测试用例的评分记录
- [x] 有 3 个维度的量化评分
- [x] 至少一个 case 的分析报告（为什么好/为什么差）
- [x] 尝试调整 chunk_size 或检索数量，观察评分变化
...
**验收标准**：
- [x] 端到端跑通，能回答问题
- [x] 遇到文档中没有的内容时，明确说"不知道"
- [x] 有评估报告和量化指标（20 个测试用例，15/20 通过，75%）
- [x] 代码结构清晰：索引、检索、生成三个阶段分离（`Indexer`, `Retriever`, `Generator` 类）
- [x] 能解释你的 chunking 策略选择和原因（按 Markdown 标题切分，语义完整，适合结构化笔记）
