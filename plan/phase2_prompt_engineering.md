...
**Milestone 1**：
- [x] 对同一个任务写 3 个不同质量等级的 prompt，输出质量有明显差异
- [x] 能列出 prompt 的 5 个核心要素并逐一解释作用
- [x] 总结出自己在实际开发中最常用的 3 种 prompt 模板
...
**Milestone 2**：
- [x] 实现一个意图分类器，zero-shot 和 few-shot 各跑 10 个测试用例
- [x] 统计两种方式下分类正确的数量，记录差异
- [x] 尝试 1-shot、3-shot、5-shot，观察准确率变化趋势
...
**Milestone 3**：
- [x] 选 5 道需要推理的题目，分别用直接回答和 CoT 两种方式
- [x] 理解 CoT 在简单任务上可能不如 Direct 模式，在复杂任务上才体现优势
- [x] 能解释为什么 CoT 有效（给了模型更多"思考 token"，减少跳跃式错误）
- [x] 知道 CoT 的代价（更多 token，更慢）
...
**Milestone 6**：
- [x] 完成至少 3 轮 prompt 迭代
- [x] 有明确的量化指标（如准确率从 60% → 85%）
- [x] 能总结自己最常用的 2-3 个 prompt 调优技巧
...
**验收标准**：
- [x] `python code_reviewer.py < code.py` 输出合法 JSON
- [x] JSON 结构符合定义的 schema
- [x] 能识别出至少 3 种不同类型的代码问题
- [x] 有测试报告和迭代记录
- [x] 代码中有清晰注释说明 prompt 设计思路
